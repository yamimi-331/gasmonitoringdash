...
def load_and_preprocess_data():
    ...
    """
    # 데이터 로드
    df = pd.read_excel("./data/TotalData2.xlsx")
...
def prepare_time_series_data(df):
    ...
    for region in df['Local'].unique():
        ...
        for i in range(len(region_data) - sequence_length):
            X_lstm.append(region_data[['TempAvg', 'HumidityAvg', 'Population']].iloc[i:i+sequence_length].values)
...
def train_models(df):
    ...
    """
    # XGBoost 모델 학습
    X = df[['Year', 'Month', 'Local', 'TempAvg', 'HumidityAvg', 'Population']]
    y = df['GasSupply']
