...
# plt.xlabel('한파일수')
# plt.ylabel('가스 공급량')
corr_cold = cold_df['ColdDay'].corr(cold_df['GasSupply'])
print(f"한파일수 vs 가스 공급량 상관계수: {corr_cold:.2f}")

sns.regplot(data=df, x='Population', y='ResidentialGas', ci=None, marker='o')
plt.title('인구수 vs 가스 공급량')
plt.xlabel('인구수')
plt.ylabel('가스 공급량')
corr_pop = df['Population'].corr(df['ResidentialGas'])
