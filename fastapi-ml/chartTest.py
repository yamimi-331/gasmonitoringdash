...
df = pd.read_excel("./data/GasSupplyData.xlsx", sheet_name='2023')
...
sns.barplot(x='Month', y='Supply', hue='Year', data=df_seoul)
plt.show()


df2=pd.read_excel("./data/WeatherData.xlsx", sheet_name='서울_전처리')
...
df2_2023 = df2_2023.sort_values('Month')

# 필요한 컬럼만 뽑아서 데이터프레임 만들기
merged = pd.merge(df_seoul[['Month', 'Supply']],
                  df2_2023[['Month', 'AvgTa']],
                  on='Month')

# 상관계수 계산 (피어슨)
correlation = merged['Supply'].corr(merged['AvgTa'])
