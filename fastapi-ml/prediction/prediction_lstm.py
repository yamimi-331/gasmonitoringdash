...
def predict_lstm_by_region(df, region_name, sequence_length=12, predict_months=3):
    ...
    try:
        ...
        # region_data = prepare_data_for_single_prediction(region_df,scaler_X, le, sequence_length)

        # if region_name in region_data:
        #     X_seq = region_data[region_name]['X_seq']
        #     print(f"[DEBUG] {region_name} 생성된 시퀀스 개수: {len(X_seq)}")
        # else:
        #     print(f"[오류] {region_name} 데이터 없음")
...
# predict_lstm_by_region 함수는 외부에서 정의되었다고 가정합니다.
# from your_module import predict_lstm_by_region
...
def get_prediction_json3(df, region_name, future_predict_months=3, recent_actual_months=9, sequence_length=12):
    """
    실제 데이터 + 과거 예측 + 미래 예측 데이터를 날짜별로 묶어 JSON 형태로 반환
    ...
    if not is_continuous_monthly(region_df['Date']):
        ...
        # 이 경우에도 진행은 하지만, 예측 신뢰도가 낮아질 수 있음을 인지해야 합니다.
    ...
    if len(region_df) < sequence_length + recent_actual_months:
        ...
        # 데이터가 너무 적으면 일부 예측이 None이 될 수 있음을 사용자에게 알림
    ...
    for i in range(len(recent_actual_df)):
        ...
        # 디버그 출력
        # print(f"[DEBUG_PAST] 예측 대상 시퀀스 날짜: {seq_start_date.strftime('%Y-%m')} ~ {seq_end_date.strftime('%Y-%m')}")
        # print(f"[DEBUG_PAST] 추출된 slice_df 길이: {len(slice_df)}")
    ...
    # print(f"[DEBUG] 최종 past_preds 길이: {len(past_preds)}") # 디버그용
    ...
    # 실제 + 과거예측 (recent_actual_months 기간)
    # recent_actual_df에 있는 실제 데이터와, 그에 대응하는 past_preds를 매핑
